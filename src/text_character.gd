...
func apply_glow(glow_color: Color, intensity: float, glow_size: float, softness: float):
	if not _glow_shader:
		_glow_shader = load(Enum.get_shader_path(Enum.EnumShaderType.GLOW))
	...
	mat.set_shader_parameter("glow_size", glow_size)
...
func apply_gradient(color_start: Color, color_end: Color, animate: bool = false, speed: float = 1.0):
	if not _gradient_shader:
		_gradient_shader = load(Enum.get_shader_path(Enum.EnumShaderType.GRADIENT))
